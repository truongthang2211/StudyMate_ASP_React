...
const columns2 = [
  {
    selector: (row) => (
      <img
        ...
        width="184px"
        height="130px"
        style={{ margin: "16px 0", borderRadius: "5px" }}
      />
    ),
    width: "216px",
    compact: true,
  },
  {
    name: "Tên khóa học",
    selector: (row) => (
      <Link to={`/course/${row.courseid}`}>{row.coursetitle}</Link>
    ),
  },
  {
    name: "Ngày đăng ký",
  ...
  },
  {
    name: "Hành động",
    selector: (row) => (
      <Link to={"edit/" + row.courseid} target="_blank">
    ...
    <>
      <div id="course-manage">
        <Sidebar User={props.User}>
          ...
          <Link
            to="/course-manage/registeredcourse"
            className="sidebar__feature-item"
          >
        ...
        <div className="course-manage-content">
          ...
          {feature == "registeredcourse" && (
            <RegisteredCourse Search={handleSearch} />
          )}
    ...
    if (data) {
      ...
      setInfoToday({
        Revenue: data["payments"]
          .filter(
            (item) =>
              moment(item["enroll_time"]).format("DD/MM/YYYY") ==
              moment().format("DD/MM/YYYY")
          )
          .reduce((a, b) => a + b.amount, 0),
        Register: data["enrollments"].filter(
          (item) =>
            moment(item["enroll_time"]).format("DD/MM/YYYY") ==
            moment().format("DD/MM/YYYY")
        ).length,
        Learnt: data["learns"].filter(
          (item) =>
            moment(item["learn_time"]).format("DD/MM/YYYY") ==
            moment().format("DD/MM/YYYY")
        ).length,
      });
    }
  ...
  return (
    <div className="course-manage-overview">
      ...
      <div className="anal-infos">
        <AnalysisItemInfo
          title="Doanh thu của bạn"
          content={formatNumber(InfoToday.Revenue) + " VNĐ"}
          time="Hôm nay"
          className="anal-item-custom"
        />
        <AnalysisItemInfo
          title="Số người đăng ký"
          content={InfoToday.Register}
          time="Hôm nay"
          className="anal-item-custom"
        />
        <AnalysisItemInfo
          title="Bài học đã học"
          content={InfoToday.Learnt}
          time="Hôm nay"
          className="anal-item-custom"
        />
      </div>
...
function RegisteredCourse(props) {
  ...
  const updateData = async () => {
    const res = await axios.get(
      "https://localhost:7074/course/get-registered-courses"
    );
    console.log(res);
    setData(res.data.message);
  };
  ...
  const coursemanageColumn = [
    ...
    },
    {
      name: "Tên khóa học",
      selector: (row) => (
        <Link to={"/course/" + row.course_id} target="_blank">
          {row.course_name}
        </Link>
      ),
      sortable: true,
    },
    {
      name: "Tác giả",
      selector: (row) => (
        <Link to={"/profile/" + row.author_id} target="_blank">
          {row.fullname}
        </Link>
      ),
      sortable: true,
...
function MyCourseApproving() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await axios.get(
          "https://localhost:7074/course/my-course-app"
        );
        setData([...res.data.message]);
        console.log(res);
      } catch (error) {
