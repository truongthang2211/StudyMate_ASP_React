    ...
    if (props.checkCommented) {
      if (!voteUp && !voteDown) {
        ...
        const res = await axios.post(
          "https://localhost:7074/Course/add-review",
          data
        );
        console.log(res);
          ...
          <ul>
            <li onClick={handleVoteUp}>
              <i
                className={
                  voteUp
                    ? "far fa-thumbs-up like active"
                    : "far fa-thumbs-up like"
                }
              ></i>
            </li>
            <li onClick={handleVoteDown}>
              <i
                className={
                  voteDown
                    ? "far fa-thumbs-down dislike active"
                    : "far fa-thumbs-down dislike"
                }
              ></i>
            </li>
            <li>
              <button onClick={handleSubmit} className="btn btn-review">
                Đánh giá
              </button>
            </li>
          </ul>
      ...
      <div className="review-content">
        ...
        <div className="review-footer">
          <i
            className={
              props.state === 1
                ? "far fa-thumbs-up like-color"
                : "far fa-thumbs-down dislike-color"
            }
          ></i>
        </div>
      </div>
...
export default function Course({ User, handleShowForm, callback }) {
    ...
    (async () => {
      try {
        const resCourse = await axios.post(
          "https://localhost:7074/Course/get-course-detail-by-id",
          { courseId: courseId }
        );
    ...
    (async () => {
      try {
        const resCheck = await axios.post(
          "https://localhost:7074/Course/check-enrolled",
          { courseId: courseId }
        );
        const flag =
          resCheck.data.message &&
          resCheck.data.message.user_id == user_id &&
          resCheck.data.message.course_id == courseId;
  ...
  const showReviews = async () => {
    try {
      const res = await axios.post(
        `https://localhost:7074/Course/get-reviews`,
        { course_id: courseId }
      );
      console.log(res);
        ...
        if (!checkPayment(courseFee, userCurrentCoin)) {
          ...
          Swal.fire({
            ...
            preConfirm: async () => {
              ...
              try {
                const resEnrollment = await axios.post(
                  "https://localhost:7074/Course/insert-enrollment",
                  enrollData
                );
              ...
              <div className="course-content">
                <div className="course-info">
                  <div className="course-summary">
                    <h1 className="course-summary-title">
                      {course && course.course_general.course_name}
                    </h1>
                  ...
                  <div className="course-gain">
                    ...
                    <div className="course-gain-list">
                      <ul className="fa-ul">
                        {course &&
                          course.course_gains.map((gain, index) => (
                ...
                <div className="course-detail">
                  ...
                  <ul className="course-detail-summary">
                    ...
                    <li>
                      {course &&
                        course.list_learn.reduce(function (total, num) {
                    ...
                    <li>
                      {course &&
                        "Thời lượng " + msecToTime(course.total_duration)}
                    </li>
                  </ul>
                  <div className="course-detail-list">
                    {course &&
                      course.list_learn.map((chapter, index) => {
                        return (
                          <Collapsible key={index} className="list">
                            <CourseChapter
                              chapterTitle={chapter.chapterTitle}
                              numOfChapters={chapter.numOfChapterLess}
                            />
                ...
                <div className="course-require">
                  ...
                  <ul className="fa-ul">
                    {course &&
                      course.course_requires.map((require, index) => (
                ...
                <div className="course-reviews">
                  ...
                  {checkEnrolled ? (
                    <InputReviewBlock
                      ...
                      checkCommented={
                        reviews &&
                        reviews.filter((e) => e.user.user_id == user_id)
                          .length == 0
                          ? true
                          : false
                      }
            ...
            <div className="grid__column-4">
              <div className="course-purchase">
                <div className="course-thumb">
                  <img
                    src={course && "/" + course.course_general.img}
                    alt="course-thumb"
                    className="center"
                  />
                </div>
                <h5 className="course-fee">
                  {course && course.course_general.fee == 0
                    ? "Miễn phí"
                    : course && modifyCourseFee(course.course_general.fee)}
                </h5>
                <Link
                  to={
                    checkEnrolled || checkOwner
                      ? "/learn/" + courseId
                      : "/course/" + courseId
                  }
                  onClick={
                ...
                <ul>
                  ...
                  <li>
                    ...
                    <span>
                      Thời lượng{" "}
                      <strong>
                        {course && msecToTime(course.total_duration)}
                      </strong>
                    </span>
                  </li>
                  <li>
                    ...
                    <span>
                      ...
                      <strong>
                        <Link
                          to={
                            course
                              ? "/profile/" + course.course_general.author
                              : ""
                          }
                        >
