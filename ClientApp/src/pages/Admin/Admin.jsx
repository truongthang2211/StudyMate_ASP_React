...
import AnalysisBox, {
  AnalysisItemInfo,
} from "../../components/AnalysisBox/AnalysisBox";
...
export default function Admin() {
  ...
  return <>{(!Admin && <AdminLogin />) || <AdminPage User={User} />}</>;
}
...
function AdminLogin() {
  ...
  return (
    <div style={{ height: "100vh", position: "relative" }}>
      <div
        className="wrap-form-head-register"
        style={{
          position: "absolute",
          top: "50%",
          left: "50%",
        ...
        <div className="form-head-register">
          <div className="forms-container">
            <form id="signin-form">
              ...
              <button onClick={handleSignIn} className="button solid">
                Sign In
              </button>
            </form>
          </div>
...
function AdminPage({ User }) {
  ...
  const handleLogout = () => {
    document.cookie =
      "StudyMateAdmin" + "=; Path=/; Expires=Thu, 01 Jan 1970 00:00:01 GMT;";
    window.location.reload();
  };
...
function Overview() {
  const topUserColumn = [
    {
      name: "Họ tên",
      selector: (row) => (
        <Link to={`/profile/${row.user_id}`} target="_blank">
          {row.fullname}
        </Link>
      ),
      sortable: true,
  ...
  const updateOverview = async () => {
    const res = await axios.get(
      "https://localhost:7074/Admin/get-overview-admin"
    );
    console.log(res);
    ...
    for (var i = NumberLabel; i > -1; --i) {
      ...
      if (TYPE != "payments") {
        number = data[TYPE].filter(
          (item) =>
            moment(item[CreateAt], "YYYY-MM-DD HH:mm:ss").format(FormatType) ==
            thisMoment.format(FormatType)
        ).length;
      } else {
        number = data[TYPE].filter(
          (item) =>
      ...
      <div className="overview-top-info">
            ...
            <div className="card-body p-3 text-center border-shadow-box">
              <p className="text text-center" style={{ color: "green" }}>
                {!data
                  ? "Loading..."
                  : (
                      ((data.revenueToDay - data.revenueYesterDay) * 100) /
                      (data.revenueToDay + data.revenueYesterDay == 0
                        ? 1
              ...
              <div className="h1 m-0">
                {!data ? "Loading..." : formatNumber(data.revenueToDay)} đ
              </div>
              <p className="text  mb-4" color="gray">
                Doanh thu hôm nay
              </p>
            </div>
            <div className="card-body p-3 text-center border-shadow-box">
              <p className="text text-center" style={{ color: "green" }}>
                {!data
                  ? "Loading..."
                  : (
                      ((data.userToDay - data.userYesterDay) * 100) /
                      (data.userToDay + data.userYesterDay == 0
                        ? 1
                        : data.userToDay + data.userYesterDay)
                    ).toFixed(2)}
                %
              </p>
              <div className="h1 m-0">
                {!data ? "Loading..." : data.userToDay}
              </div>
              <p className="text  mb-4" color="gray">
                Người dùng hôm nay
              </p>
            ...
            <div className="card-body p-3 text-center border-shadow-box">
              <p className="text text-center" style={{ color: "green" }}>
                {(
                  ...
                  (CourseToday + CourseYesterday == 0
                    ? 1
                    : CourseToday + CourseYesterday)
                ).toFixed(2)}
                %
              </p>
            ...
            <div className="card-body p-3 text-center border-shadow-box">
              <p className="text text-center" style={{ color: "green" }}>
                {!data
                  ...
                  : (
                      (data.revenueToDay - data.revenueYesterDay) /
                      (data.totalRevenue == 0 ? 1 : data.totalRevenue)
                    ).toFixed(4)}
                %
              </p>
              <div className="h1 m-0">
                {!data ? "Loading..." : formatNumber(data.totalRevenue)} đ
              </div>
              <p className="text  mb-4" color="gray">
                Tổng doanh thu
              </p>
        ...
        <div className="overview-info-right">
          <AnalysisBox Data={dataanal}>
            <span>
              {TypeContent} trong {TimeContent}: {formatNumber(TotalContent)}
            </span>
            <select
              value={timevalue}
...
const usermanageColumn = [
  ...
  },
  {
    ...
    selector: (row) => (
      <UpdateAction
        view={`/profile/${row.user_id}`}
        edit={"edit/" + row.user_id}
      />
    ),
    minWidth: "200px",
  },
...
function Approval(props) {
  ...
  const updateapp = async () => {
    const res = await axios.get(
      "https://localhost:7074/Admin/get-list-createapp"
    );
    console.log(res);
    setPending(false);
  ...
  const approvalColumn = [
    ...
    },
    {
      name: "Tác giả",
      selector: (row) => (
        <Link to={`/profile/${row.author.userid}`}>{row.author.fullname}</Link>
      ),
      sortable: true,
    },
    {
    ...
    },
    {
      ...
      selector: (row) => (
        <Link
          ...
          target="_blank"
          rel="noopener"
          className="btn my-custom-button-default"
        >
...
function CourseManage(props) {
  ...
  const coursemanageColumn = [
    ...
    },
    {
      ...
      selector: (row) => (
        <UpdateAction
          view={"learn/" + row.courseid}
          callback={updateData}
          edit={"edit/" + row.courseid}
...
function Approvaled(props) {
  ...
  useEffect(() => {
    (async () => {
      const res = await axios.get(
        "https://localhost:7074/Admin/get-list-approvaled"
      );
      console.log(res);
      setData(res.data.message);
    })();
...
function ApprovalAction(props) {
    ...
    Swal.fire({
      ...
      preConfirm: async (reason) => {
        const res = await axios.post(
          "https://localhost:7074/Course/action-course",
          { _id: props._id, accept: false, reason: reason }
        );
        console.log(res);
        props.callback();
      },
  ...
  return (
    <>
      <a onClick={handleAccept} className="btn my-custom-button-default">
        Đồng ý
      </a>
      <a onClick={handleRefuse} className="btn my-custom-button-default">
        Từ chối
      </a>
    </>
  );
...
function UpdateAction(props) {
  const lock_Classname =
    props.state == "Công khai"
      ? "fas fa-lock"
      : props.state == "Bị khóa"
      ? "fas fa-lock-open"
      : "";
  const handleLock = () => {
    if (props.state == "Công khai") {
      Swal.fire({
        ...
        preConfirm: async (reason) => {
          const res = await axios.post(
            "https://localhost:7074/Admin/lock-course-action",
            { course_id: props.course_id, reason: reason }
          );
          console.log(res);
          props.callback();
        },
  ...
  return (
    <>
      ...
      <a
        onClick={handleLock}
        className={lock_Classname ? "btn my-custom-button-default" : "d-none"}
      >
        <i className={lock_Classname}></i>
      </a>
    </>
  );
}
