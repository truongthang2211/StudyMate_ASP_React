...
const ThisUserID = new URLSearchParams(
...
export default memo(function Learn({ LearnData, Admin, User }) {
  ...
  const onClickRepl = (index, parentindex, repl, parent) => {
    ...
    if (repl) {
      ...
      if (!sub_comment || !sub_comment["thisuser"])
        newcomments[parentindex]["subcomments"].splice(index + 1, 0, {
          thisuser: true,
          parent_comment: parent,
        });
    } else if (
      !newcomments[index]["subcomments"][0] ||
      !newcomments[index]["subcomments"][0]["thisuser"]
    ) {
      newcomments[index]["subcomments"].splice(0, 0, {
        thisuser: true,
        parent_comment: parent,
      });
    }
      ...
      try {
        ...
        if (!LearnData) {
          const res = await axios.get(
            `https://localhost:7074/Course/get-learn/${Tcourse}/${Tlesson}`
          );
          console.log(res);
          ...
          if (!Admin) {
            if (res.data.message.lastlessonlearnt === -1) {
              navigate(
                `/learn/${Tcourse}/${res.data.message.listlearn[0].lesson[0].lesson_id}`
              );
            } else if (!lesson) {
              navigate(
                `/learn/${Tcourse}/${res.data.message.lastlessonlearnt + 1}`
              );
            } else {
  ...
  }, [Admin, LearnData, course, feature, id, lesson, navigate, subid]);
  const showComment = useCallback(async () => {
    if (feature !== "approval") {
      try {
        const res = await axios.post(
          `https://localhost:7074/comment/get-comments`,
          { lesson_id: lesson || subid }
        );
        console.log(res);
  ...
  }, [feature, lesson, subid]);
  useEffect(() => {
    if (lesson || subid) {
      showComment();
    }
  }, [lesson, showComment, subid]);
    ...
    if (e.data === 1 && !Admin && ThisUserID !== dataLearning.author) {
      TimerId = setInterval(() => {
        if (e.target.getCurrentTime() / e.target.getDuration() > 0.8) {
          if (
            dataLearning.lastlessonlearnt + 1 === lesson ||
            dataLearning.lastlessonlearnt === -1
          ) {
            ...
            axios
              .post("https://localhost:7074/course/add-learn", {
    ...
    !pending && (
      <>
        <div id="left-learning">
          <div className="breadcrumb">
            ...
            <Link
              ...
              to={`/list-course/${
                dataLearning.coursemaintype &&
                dataLearning.coursemaintype.course_maintype_id
              }`}
            >
              {dataLearning.coursemaintype &&
                dataLearning.coursemaintype.type_name}
            </Link>
            <Link
              className="breadcrumb-item"
              to={`/list-course/null/${
                dataLearning.coursetype &&
                dataLearning.coursetype.course_subtype_id
              }`}
            >
              {dataLearning.coursetype && dataLearning.coursetype.type_name}
            </Link>
            <span className="breadcrumb-item active">
              {dataLearning.coursetitle}
            </span>
          </div>
                        ...
                        return (
                          <li key={index}>
                            <Comment
                              ...
                              parentComment={item.commentid}
                              UsersVoted={item.usersvoted}
                              Time={item.commenttime}
                              handleRepl={onClickRepl}
                              commentID={item.commentid}
                              updateComment={showComment}
                            />
                              ...
                              if (item2.thisuser) {
                                return (
                                  <UserComment
                                    ...
                                    index={index2}
                                    repl
                                    parentComment={item2.parent_comment}
                                ...
                                return (
                                  <Comment
                                    ...
                                    key={index2}
                                    parentComment={item.commentid}
                                    UsersVoted={item2.usersvoted}
                                    updateComment={showComment}
                                    Time={item2.commenttime}
                                    repl
                                    User={item2.user}
        ...
        <div id="right-learning">
          <RightHeader
            learnt={
              dataLearning.listlearn &&
              dataLearning.listlearn.reduce(
                (a, b) =>
                  a +
                  b.lesson.filter(
                    (e) => e.lesson_id <= dataLearning.lastlessonlearnt
                  ).length,
                0
              )
            }
            totalLesson={
              dataLearning.listlearn &&
              dataLearning.listlearn.reduce((a, b) => a + b.lesson.length, 0)
            }
            title={dataLearning.listlearn && dataLearning.coursetitle}
          />
          {dataLearning.listlearn &&
            dataLearning.listlearn.map((item, index) => {
              return (
                <Collapsible className="playlist-wrapper" key={index}>
                  <Chapter
                    ...
                    totalLesson={item.lesson.length}
                    Duration={msecToTime(
                      item.lesson.reduce((a, b) => a + b.duration, 0)
                    )}
                  />
                  <div className="playlist-wrapper-list">
                    {item.lesson.map((less, index2) => {
                      if (
                        less.lesson_id === dataLearning.lastlessonlearnt + 1 ||
                        (dataLearning.lastlessonlearnt === -1 &&
                          less.lesson_id === lesson) ||
                        dataLearning.author === ThisUserID ||
                        Admin
                      ) {
                        status = "normal-item";
                      } else if (
                      ...
                      if (
                        (!Admin && less.lesson_id === lesson) ||
                        less.lesson_id === subid ||
                        subid === index2
                      ) {
                      ...
                      return (
                        <Lession
                          ...
                          lesson_id={less.lesson_id ?? index2}
                          videoURL={less.lesson_url}
                          key={index2}
                          title={less.lesson_name}
                          duration={msecToTime(less.duration)}
                        />
                      );
                    })}
                  </div>
...
function UserComment(props) {
  ...
  const ClassName = props.repl
    ? "comment-block comment-block-repl"
    : "comment-block";
  ...
  const handleSubmit = async () => {
    const data = {
      lesson_id: parseInt(lesson),
      content: commentData,
      parent_comment_id: props.parentComment
        ? parseInt(props.parentComment)
        : null,
    };
    setComment("");
    const res = await axios.post(
      "https://localhost:7074/comment/add-comment",
      data
    );
    console.log(res);
    ...
    <div id="user-comment-block">
      <div className={ClassName}>
        <div className="comment-avt">
          <img
            className="CommentBox_myAvatar__3Mi09"
            src={
              props.User && props.User.AVATAR_IMG
                ? `/${props.User.AVATAR_IMG}`
                : "https://genk.mediacdn.vn/thumb_w/600/2015/screen-shot-2015-07-30-at-2-31-57-pm-1438334096188.png"
            }
            alt="Thang Nguyen"
          />
        </div>
        <div className="comment-content align-items-end">
          ...
          <div>
            ...
            {props.repl && (
              <button
                onClick={() =>
                  props.handleCancel(props.index, props.parent_index)
                }
...
function Comment(props) {
  const [vote, setVote] = useState(() => {
    ...
    if (filter.length) {
      return {
        up: filter[0].comment_vote_state === 1,
        down: filter[0].comment_vote_state === 0,
      };
    }
  ...
  const handleUpVote = () => {
    ...
    console.log(data);
    axios
      .post("https://localhost:7074/comment/comment-vote", data)
      .then((r) => {
        console.log(r);
        props.updateComment();
      })
        ...
        <div className="comment-footer">
          <span className="comment-vote">
            <span className={"upvote " + (vote.up ? "active" : "")}>
              ...
              <span>
                {
                  props.UsersVoted.filter((e) => e.comment_vote_state === 1)
                    .length
                }
              </span>
            </span>
            <span className={"downvote " + (vote.down ? "active" : "")}>
              ...
              <span>
                {
                  props.UsersVoted.filter((e) => e.comment_vote_state === 0)
                    .length
                }
              </span>
            </span>
...
function Chapter(props) {
  return (
    <div className="playplist-wrapper-header">
      ...
      <p className="wrapper-header-detail">
        {props.learnt}/{props.totalLesson} | {props.Duration}
      </p>
    </div>
  );
}
function Lession({
  ...
  return (
    <div
      onClick={() => handleLesson(videoURL, lesson_id, status)}
      className={"playlist-wrapper-item " + status}
    >
...
function RightHeader(props) {
  return (
    <header
      ...
      style={{
        "--process":
          ((props.learnt / props.totalLesson) * 100).toFixed(2) + "%",
      }}
    >
      <div className="background-process"></div>
      <div className="playlist-header-content">
        <h1 className="playlist-title">{props.title}</h1>
        <div className="playlist-info">
          <p className="playlist-description">
            Hoàn thành
            <strong> {props.learnt}</strong>/
            <strong>{props.totalLesson} </strong>
            bài học (
            <strong>
              {((props.learnt / props.totalLesson) * 100).toFixed(2)} %
            </strong>
